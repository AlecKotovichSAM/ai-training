...
class RetentionServiceIntegrationTest {
    ...
    void archivesExpiredEvents() {
        ...
        assertThat(archiveRepo.findAll().stream()
                .anyMatch(a -> a.getActor().equals(uniqueActor))).isTrue();
    }
    ...
    void idempotentOnRepeatedRuns() {
        ...
        retentionService.archiveExpiredEvents();
        long archiveCountAfterFirst = archiveRepo.findAll().stream()
                .filter(a -> a.getActor().equals(uniqueActor)).count();

        // Second run must not throw and must not create duplicate archive records
        retentionService.archiveExpiredEvents();
        long archiveCountAfterSecond = archiveRepo.findAll().stream()
                .filter(a -> a.getActor().equals(uniqueActor)).count();
