...
public record CreateAuditEventRequest(
        ...
        @Schema(description = "Identity of the user or service that performed the action", example = "user:42")
        @NotBlank(message = "actor is required")
        String actor,

        @Schema(description = "Name of the action that was performed", example = "invoice.updated")
        @NotBlank(message = "action is required")
        String action,

        @Schema(description = "Resource that was affected by the action", example = "invoice:777")
        String resource,
        ...
        @Schema(description = "Additional contextual key-value pairs", example = "{\"ip\": \"10.0.0.1\"}")
