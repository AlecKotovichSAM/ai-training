 ...
 * guarantees a stable response contract — independent of Spring Data's
 * internal serialisation changes.
 * </p>
 */
@Schema(description = "Paginated response wrapper with a stable JSON structure")
public record PageResponse<T>(

        @Schema(description = "Page content (items on the current page)")
        List<T> content,
        ...
        @Schema(description = "Number of elements on the current page", example = "50")
        int numberOfElements,

        @Schema(description = "Total number of elements across all pages", example = "1234")
        long totalElements,
