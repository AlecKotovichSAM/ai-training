...
- В `README.md` раздел `## 📅 Change History` вести в обратной хронологии:
  самые новые даты и записи размещать сверху.
- Перед обновлением `Change History` собирать факты через git-команды
  (`git status --short`, `git diff --name-only`, `git log --oneline -N`),
  и писать только то, что подтверждается текущим diff/коммитами.
- При любых изменениях кода использовать настройки форматирования Spotless из `pom.xml`
  (Eclipse formatter с кастомным конфигом) как единственный источник формат-правил.
